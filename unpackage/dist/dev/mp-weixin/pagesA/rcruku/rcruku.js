...
var _rcruku = _interopRequireDefault(__webpack_require__(/*! ./pagesA/rcruku/rcruku.vue */ 303));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // @ts-ignore
wx.__webpack_require_UNI_MP_PLUGIN__ = __webpack_require__;createPage(_rcruku.default);
...
/* harmony import */ var _rcruku_vue_vue_type_template_id_0fcf70aa_scoped_true___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./rcruku.vue?vue&type=template&id=0fcf70aa&scoped=true& */ 304);
/* harmony import */ var _rcruku_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./rcruku.vue?vue&type=script&lang=js& */ 306);
...
/* harmony import */ var _rcruku_vue_vue_type_style_index_0_id_0fcf70aa_lang_scss_scoped_true___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./rcruku.vue?vue&type=style&index=0&id=0fcf70aa&lang=scss&scoped=true& */ 308);
...
//
//
var navbar = function navbar() {__webpack_require__.e(/*! require.ensure | components/nav */ "components/nav").then((function () {return resolve(__webpack_require__(/*! ../../components/nav.vue */ 197));}).bind(null, __webpack_require__)).catch(__webpack_require__.oe);};var _default = { components: { navbar: navbar }, data: function data() {return { authority: true, spbm: '', sjbh: '', sjbhlist: '', thck: '', //仓库
      thcklist: '', thlx: '', //退回类型
      thlxlist: '', rkdh: uni.getStorageSync('rkdh'), //入货单
      thrq: '', //退货日期
      detail: true, //明细
      detaildata: [], //明细数据
      from: {}, popupShow: false, searchdata: '', pitchdata: '', //选中
      remark: '', //备注
      shcg: false };}, onLoad: function onLoad(option) {console.log(option);if (option.rkdh) {uni.setStorageSync('rkdh', option.rkdh);this.rkdh = option.rkdh;}}, onReady: function onReady() {}, onShow: function onShow() {var authority = uni.getStorageSync('authority').split(';');if (authority.indexOf('999921') != '-1') {this.authority = false;} //处理仓库下拉框数据
    ...
    this.thcklist = uni.getStorageSync('basic').CKINFO;var thcklist = [];this.thcklist.forEach(function (item) {var datas = {};datas.value = item.ckbmid;datas.text = item.ckmc;thcklist.push(datas);});this.thcklist = thcklist;this.thck = this.thcklist[0].value;if (uni.getStorageSync('rkdh')) {this.getlist();} else {this.cknew();}}, methods: { //退出
    back: function back() {uni.switchTab({ url: '../../pages/statement/statement' });}, //创建出库单
    cknew: function cknew() {var _this = this;var data = { access_token: uni.getStorageSync('access_token'), djtype: 'SPRKD', fdbh: uni.getStorageSync('fdbh'), userid: uni.getStorageSync('userid') };(0, _api.rcOrderNew)(data).then(function (res) {console.log('入库单创建成功', res);_this.shcg = false;_this.rkdh = res.djbh;uni.setStorageSync('rkdh', _this.rkdh);var datee = _this.rkdh.split("RK")[1];var y = "20" + datee.slice(0, 2);var m = datee.slice(2, 4);var d = datee.slice(4, 6);_this.thrq = "".concat(y, "-").concat(m, "-").concat(d);});}, // 扫码 搜索商品
    ...
    Search: function Search() {var _this3 = this;if (this.spbm.length < 4) {uni.showToast({ title: '商品条码至少输入四位', duration: 2000, icon: 'none' });return;}if (this.rkdh) {var data = { access_token: uni.getStorageSync('access_token'), companyid: uni.getStorageSync('companyid'), condition: this.spbm, fdbh: uni.getStorageSync('fdbh'), findtype: '01', goodstype: 'SP', userid: uni.getStorageSync('userid') };(0, _api.rcsearch)(data).then(function (res) {console.log(res);if (res.error_code == '0') {console.log('搜索到的', res);if (res.data.length > '0' && res.data.length != '1') {_this3.popupShow = true;_this3.searchdata = res.data;}if (res.data.length == '1') {console.log('只有一个', res.data[0]);_this3.ispitchdata(res.data[0]);}
          }
