...
var _rcruku = _interopRequireDefault(__webpack_require__(/*! ./pagesA/rcruku/rcruku.vue */ 327));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // @ts-ignore
...
/* harmony import */ var _rcruku_vue_vue_type_template_id_0fcf70aa_scoped_true___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./rcruku.vue?vue&type=template&id=0fcf70aa&scoped=true& */ 328);
/* harmony import */ var _rcruku_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./rcruku.vue?vue&type=script&lang=js& */ 330);
...
/* harmony import */ var _rcruku_vue_vue_type_style_index_0_id_0fcf70aa_lang_scss_scoped_true___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./rcruku.vue?vue&type=style&index=0&id=0fcf70aa&lang=scss&scoped=true& */ 332);
...
var navbar = function navbar() {__webpack_require__.e(/*! require.ensure | components/nav */ "components/nav").then((function () {return resolve(__webpack_require__(/*! ../../components/nav.vue */ 197));}).bind(null, __webpack_require__)).catch(__webpack_require__.oe);};var _default = { components: { navbar: navbar }, data: function data() {return { spbm: '', sjbh: '', sjbhlist: '', thck: '', //仓库
      ...
      remark: '', //备注
      shcg: false };}, onLoad: function onLoad(option) {console.log(option);if (option.rkdh) {uni.setStorageSync('rkdh', option.rkdh);this.rkdh = option.rkdh;}}, onReady: function onReady() {}, onShow: function onShow() {//处理仓库下拉框数据
    ...
    Search: function Search() {var _this3 = this;if (this.rkdh) {var data = { access_token: uni.getStorageSync('access_token'), companyid: uni.getStorageSync('companyid'), condition: this.spbm, fdbh: uni.getStorageSync('fdbh'), findtype: '01', goodstype: 'SP', userid: uni.getStorageSync('userid') };(0, _api.rcsearch)(data).then(function (res) {console.log(res);if (res.error_code == '0') {console.log('搜索到的', res);if (res.data.length > '0' && res.data.length != '1') {_this3.popupShow = true;_this3.searchdata = res.data;}if (res.data.length == '1') {console.log('只有一个', res.data[0]);_this3.ispitchdata(res.data[0]);}}if (res.error_code == '500') {_this3.searchdata = [];_this3.pitchdata = '';_this3.from = {};uni.showToast({ title: '未搜索到商品', duration: 2000, icon: 'none' });}});} else {
        this.cknew();
      }
